...
trait LlmsTxt {
    public function register_llmstxt_hooks() {
        if (get_option('aiutoma_markdown_enabled', '1') !== '1') return;
        if (get_option('aiutoma_markdown_llmstxt_enabled', '1') !== '1') return;
    ...
    public function add_llmstxt_rewrite_rule() {
        add_rewrite_rule('^llms\.txt$', 'index.php?aiutoma_llmstxt=1', 'top');
    }

    public function add_llmstxt_query_vars($vars) {
        $vars[] = 'aiutoma_llmstxt';
        return $vars;
    }

    public function serve_llmstxt_content() {
        if (get_query_var('aiutoma_llmstxt')) {
    ...
    private function generate_llmstxt_content() {
        ...
        $allowed_cpts = get_option('aiutoma_markdown_cpts', false);
        ...
        foreach ($allowed_cpts as $post_type) {
            ...
            if (!empty($posts)) {
                ...
                foreach ($posts as $post) {
                    ...
                    // Append .md to URL for Aiutoma Markdown module compatibility
                    $url = rtrim($url, '/') . '.md';
        ...
        $content .= "[comment]: # (Generated dynamically by Aiutoma)\n";
