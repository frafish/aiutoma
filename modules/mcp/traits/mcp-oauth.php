<?php
namespace Aiutoma\Modules\Mcp\Traits;
...
trait McpOauth {
  ...
  public const NONCE_ACTION = 'aiutoma_oauth_consent';
  ...
  public function init_mcp_oauth() {
    ...
    $this->oauth_namespace = 'aiutoma/v1';
    $this->logging = false;
    $this->table_clients = $wpdb->prefix . 'aiutoma_oauth_clients';
    $this->table_tokens = $wpdb->prefix . 'aiutoma_oauth_tokens';
    ...
    // when no X-WP-Nonce is sent. The browser-driven authorize flow is an interactive page
    // visited by authenticated administrators. Hooking at priority 90 before rest_cookie_check_errors (100)
    // preserves the existing cookie-authenticated user without needing wp_set_current_user().
    // CSRF is enforced separately via our own consent nonce on POST.
    add_filter( 'rest_authentication_errors', [ $this, 'preserve_cookie_auth_for_authorize' ], 90 );
  ...
  public function preserve_cookie_auth_for_authorize( $result ) {
    if ( ! empty( $result ) ) {
      return $result;
    }
    ...
    if ( strpos( $uri, '/' . $this->oauth_namespace . '/oauth/authorize' ) !== false ) {
      // If the user is already authenticated via WordPress session cookies, return true
      // so core's rest_cookie_check_errors (priority 100) does not downgrade the user to 0
      // due to a missing X-WP-Nonce header. The consent form handles CSRF via its own nonce.
  ...
  private function maybe_upgrade_db() {
    if ( get_option( 'aiutoma_oauth_db_version' ) === $this->DB_VERSION ) {
      return;
    }
    ...
    update_option( 'aiutoma_oauth_db_version', $this->DB_VERSION );
  }
  #endregion
  ...
  private function handle_authorize_submit( WP_REST_Request $request ) {
    ...
    $nonce = (string) $request->get_param( '_maiutoma_nonce' );
  ...
  private function auth_code_key( $code ) {
    return 'aiutoma_oauth_code_' . hash( 'sha256', $code );
  }
  #endregion
  ...
  public function user_can_authorize( $user_id ) {
    ...
    return (bool) apply_filters( 'aiutoma_mcp_oauth_user_can_authorize', $allowed, $user_id );
  }
  #endregion
  ...
  private function render_consent_page( $client, $params, $user ) {
    ...
    $hidden_fields = [
      ...
      '_maiutoma_nonce' => $nonce,
    ];

    echo '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">';
    echo '<meta name="viewport" content="width=device-width, initial-scale=1">';
    echo '<title>' . esc_html( sprintf( 'Authorize %s', $client_name ) ) . '</title>';
    $this->print_consent_styles();
    echo '</head><body><main class="maiutoma-oauth-card">';

    echo '<h1>Authorize this app</h1>';
    echo '<p class="maiutoma-oauth-app"><strong>' . esc_html( $client_name ) . '</strong> wants to connect to <strong>' . esc_html( $site_name ) . '</strong>.</p>';

    echo '<div class="maiutoma-oauth-meta">';
    echo '<div><span class="maiutoma-oauth-label">Signed in as</span><span class="maiutoma-oauth-value">' . esc_html( $user->display_name ) . ' (' . esc_html( $user->user_login ) . ')</span></div>';
    echo '<div><span class="maiutoma-oauth-label">Permissions</span><span class="maiutoma-oauth-value">' . esc_html( $role_label ) . '</span></div>';
    echo '</div>';

    echo '<p class="maiutoma-oauth-note">The app will be able to call MCP tools using your account. You can revoke access at any time from AI Engine settings.</p>';

    echo '<form method="POST" action="' . esc_url( $action_url ) . '">';
    foreach ( $hidden_fields as $name => $value ) {
      echo '<input type="hidden" name="' . esc_attr( $name ) . '" value="' . esc_attr( $value ) . '">';
    }
    echo '<div class="maiutoma-oauth-buttons">';
    echo '<button type="submit" name="action" value="approve" class="maiutoma-oauth-approve">Approve</button>';
    echo '<button type="submit" name="action" value="deny" class="maiutoma-oauth-deny">Deny</button>';
    echo '</div>';
    echo '</form>';
  ...
  private function render_error_page( $message ) {
    ...
    echo '<p class="maiutoma-oauth-note">' . esc_html( $message ) . '</p>';
    echo '</main></body></html>';
  }
  ...
  private function print_consent_styles() {
    wp_register_style( 'aiutoma-mcp-oauth', AIUTOMA_URL . 'modules/mcp/assets/css/mcp-oauth.css', [], AIUTOMA_VERSION );
    wp_print_styles( 'aiutoma-mcp-oauth' );
  }
  #endregion
}
