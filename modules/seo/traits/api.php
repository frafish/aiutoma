...
trait Api {
    public function api_save_seo_settings(\WP_REST_Request $request) {
        if ($request->has_param('auto_optimize')) {
            update_option('aiutoma_auto_optimize_media', $request->get_param('auto_optimize'));
        }
        if ($request->has_param('preferred_model')) {
            update_option('aiutoma_seo_preferred_model', $request->get_param('preferred_model'));
        }
        if ($request->has_param('text_model')) {
            update_option('aiutoma_seo_text_model', $request->get_param('text_model'));
        }

        if ($request->has_param('markdown_enabled')) {
            update_option('aiutoma_markdown_enabled', $request->get_param('markdown_enabled') === 'true' ? '1' : '0');
            update_option('aiutoma_markdown_llmstxt_enabled', $request->get_param('markdown_llmstxt_enabled') === 'true' ? '1' : '0');
            $cpts = $request->get_param('markdown_cpts');
            update_option('aiutoma_markdown_cpts', is_array($cpts) ? array_map('sanitize_text_field', $cpts) : []);
            flush_rewrite_rules();
        }
...
Respond ONLY with a valid JSON object strictly matching this schema, without markdown formatting or preamble:
{
    ...
    \"caption\": \"A short caption that could be displayed under the image\"
        ...
        wp_save_post_revision($attachment_id);
        ...
        update_post_meta($attachment_id, '_aiutoma_seo_optimized', time());
...
" . $content;
        ...
        update_post_meta($post_id, '_aiutoma_seo_optimized', time());
