...
class AI_Request_Log_Repository {
	 ...
	 */
	private const CACHE_GROUP = 'aiutoma_request_logs';
	...
	public function insert( array $data ) {
		 ...
		 */
		do_action( 'aiutoma_request_logged', $log_id, $insert_data );
	...
	public function get_summary( string $period = 'day', bool $force_refresh = false ): array {
		...
		$sql = "SELECT
			...
			GROUP BY type, provider, status"; // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
	...
	public function get_filter_options( bool $force_refresh = false ): array {
		...
		$sql = "SELECT 'type' as category, type as value FROM {$table_name} WHERE type IS NOT NULL GROUP BY type
				...
				UNION ALL
				SELECT 'user' as category, user_id as value FROM {$table_name} WHERE user_id IS NOT NULL AND user_id > 0 GROUP BY user_id"; // phpcs:ignore PluginCheck.Security.DirectDB.UnescapedDBParameter
