...
namespace Aiutoma\Modules\Ai\Classes\TokensLog\REST;
...
class AI_Request_Log_Controller extends WP_REST_Controller {
	...
	public function get_log( WP_REST_Request $request ) {
		...
		if ( ! $log ) {
			return new WP_Error(
				'aiutoma_log_not_found',
				__( 'Log entry not found.', 'aiutoma' ),
				array( 'status' => 404 )
			);
		}
		...
		return rest_ensure_response(
			array(
				...
				'message' => sprintf(
					...
					__( 'Successfully purged %d log entries.', 'aiutoma' ),
					$deleted
				),
			)
		);
	...
	public function get_collection_params(): array {
		return array(
			'type'             => array(
				'description' => __( 'Filter by log type.', 'aiutoma' ),
				'type'        => 'string',
				'enum'        => array( '', 'ai_client', 'mcp_tool', 'ability' ),
				'default'     => '',
			),
			'status'           => array(
				'description' => __( 'Filter by status.', 'aiutoma' ),
				'type'        => 'string',
				'enum'        => array( '', 'success', 'error', 'timeout' ),
				'default'     => '',
			),
			'provider'         => array(
				'description' => __( 'Filter by AI provider.', 'aiutoma' ),
				'type'        => 'string',
				'default'     => '',
			),
			'user_id'          => array(
				'description' => __( 'Filter by user ID.', 'aiutoma' ),
				'type'        => 'integer',
				'default'     => 0,
			),
			'date_from'        => array(
				'description' => __( 'Filter logs from this date (YYYY-MM-DD HH:MM:SS).', 'aiutoma' ),
				'type'        => 'string',
				'format'      => 'date-time',
			),
			'date_to'          => array(
				'description' => __( 'Filter logs until this date (YYYY-MM-DD HH:MM:SS).', 'aiutoma' ),
				'type'        => 'string',
				'format'      => 'date-time',
			),
			'search'           => array(
				'description' => __( 'Search in operations, request previews, response previews, and error messages.', 'aiutoma' ),
				'type'        => 'string',
				'default'     => '',
			),
			'tokens_filter'    => array(
				'description' => __( 'Filter by tokens: "gt:N", "lt:N", or "none".', 'aiutoma' ),
				'type'        => 'string',
				'default'     => '',
			),
			'page'             => array(
				'description' => __( 'Current page of the collection.', 'aiutoma' ),
				'type'        => 'integer',
				'default'     => 1,
				'minimum'     => 1,
			),
			'per_page'         => array(
				'description' => __( 'Maximum number of items per page.', 'aiutoma' ),
				'type'        => 'integer',
				'default'     => 25,
				'minimum'     => 1,
				'maximum'     => 100,
			...
			'orderby'          => array(
				'description' => __( 'Sort collection by attribute.', 'aiutoma' ),
				'type'        => 'string',
				'enum'        => array( 'timestamp', 'type', 'operation', 'duration_ms', 'tokens_total', 'status' ),
				'default'     => 'timestamp',
			),
			'order'            => array(
				'description' => __( 'Order sort attribute ascending or descending.', 'aiutoma' ),
				'type'        => 'string',
				'enum'        => array( 'ASC', 'DESC' ),
				'default'     => 'DESC',
			),
			'cursor_id'        => array(
				'description' => __( 'Cursor ID for cursor-based pagination (use with cursor_timestamp).', 'aiutoma' ),
				'type'        => 'integer',
				'minimum'     => 1,
			),
			'cursor_timestamp' => array(
				'description' => __( 'Cursor timestamp for cursor-based pagination (use with cursor_id).', 'aiutoma' ),
				'type'        => 'string',
				'format'      => 'date-time',
			),
		);
