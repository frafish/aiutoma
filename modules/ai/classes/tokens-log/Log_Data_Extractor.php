...
class Log_Data_Extractor {
	...
	/**
	 * Provider URL patterns used for provider detection.
	 *
	 * Filterable via 'aiutoma_request_log_providers'.
	 *
	...
	public function __construct() {
		$this->provider_patterns = $this->get_provider_patterns();
	}

	/**
	 * Returns the provider detection patterns.
	 *
	 * @return array<string, array<string>> Map of provider name to URL patterns.
	 */
	public function get_provider_patterns(): array {
		$patterns = array(
		...
		$custom_ollama = get_option( 'ai_request_logs_ollama_host' );
		if ( $custom_ollama && is_string( $custom_ollama ) ) {
			$patterns['Ollama'][] = $custom_ollama;
		}
		 ...
		 */
		return (array) apply_filters( 'aiutoma_request_log_providers', $patterns );
	}
	...
	public function extract_response_data( ?string $body, array $log_data ): array {
		 ...
		 */
		$context = (array) apply_filters( 'aiutoma_request_log_context', $context, $decoded, $log_data );
	...
	public function detect_request_kind( ?string $provider, string $path, ?array $payload ): string {
		 ...
		 * @param array<string, mixed>|null $payload  Request payload.
		 */
		return (string) apply_filters( 'aiutoma_request_log_kind', 'text', $provider, $path, $payload );
	}
