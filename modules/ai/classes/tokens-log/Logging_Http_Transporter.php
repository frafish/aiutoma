...
class Logging_Http_Transporter implements HttpTransporterInterface {
	...
	private function is_infrastructure_file( string $file ): bool {
		...
		if ( 0 === strpos( $file, $logging_dir ) || false !== strpos( $file, '/vendor/' ) || false !== strpos( $file, '/wp-includes/php-ai-client/' ) || false !== strpos( $file, '/wp-includes/ai-client/' ) ) {
			return true;
		}
	...
	private function get_infrastructure_dirs(): array {
		$dirs        = array();
		$plugins_dir = trailingslashit( wp_normalize_path( dirname( AIUTOMA_PATH ) ) );
	...
	private function match_plugin_source( string $file ): ?array {
		...
		return array(
			'type' => 'plugin',
			'slug' => $slug,
			'name' => 'ai' === $slug ? __( 'AI', 'aiutoma' ) : $slug,
			'file' => $relative,
		);
	}
	...
	private function match_core_source( string $file ): ?array {
		if ( preg_match( '#/(wp-admin|wp-includes)/(.+)$#', $file, $matches ) ) {
			return array(
				'type' => 'core',
				'slug' => 'wordpress',
				'name' => __( 'WordPress Core', 'aiutoma' ),
