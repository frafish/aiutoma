...
if (!class_exists('Aiutoma\Modules\Ai\Traits\Aiutoma_Ability_Table')) {
    class Aiutoma_Ability_Table extends \WP_List_Table {
        ...
        public function get_columns() {
            return [
                'name'        => __('Name', 'aiutoma'),
                'slug'        => __('Slug', 'aiutoma'),
                'description' => __('Description', 'aiutoma'),
                'provider'    => __('Provider', 'aiutoma'),
                'category'    => __('Category', 'aiutoma'),
                'actions'     => __('Actions', 'aiutoma')
            ];
        }
        ...
        public function column_provider($item) {
            ...
            $label = $provider;
            if ($provider === 'Core') $label = __('Core', 'aiutoma');
            if ($provider === 'Theme') $label = __('Theme', 'aiutoma');
            if ($provider === 'Plugin') {
                $label = !empty($item['plugin_name']) ? $item['plugin_name'] : __('Plugin', 'aiutoma');
            }
        ...
        public function column_actions($item) {
            ...
            return sprintf(
                ...
                esc_url($detail_url),
                esc_html__('View', 'aiutoma'),
                esc_url($test_url),
                esc_html__('Test', 'aiutoma')
            );
        }
        ...
        public function extra_tablenav($which) {
            ...
            <div class="alignleft actions">
                <select name="provider">
                    <option value="all" <?php selected($provider_filter, 'all'); ?>><?php esc_html_e('All Providers', 'aiutoma'); ?></option>
                    <option value="Core" <?php selected($provider_filter, 'Core'); ?>><?php esc_html_e('Core', 'aiutoma'); ?></option>
                    
                    <optgroup label="<?php esc_attr_e('Plugins', 'aiutoma'); ?>">
                        <option value="Plugin" <?php selected($provider_filter, 'Plugin'); ?>><?php esc_html_e('All Plugins', 'aiutoma'); ?></option>
                    ...
                    <option value="Theme" <?php selected($provider_filter, 'Theme'); ?>><?php esc_html_e('Theme', 'aiutoma'); ?></option>
                </select>

                <select name="category">
                    <option value="all" <?php selected($category_filter, 'all'); ?>><?php esc_html_e('All Categories', 'aiutoma'); ?></option>
                ...
                <?php submit_button(__('Filter', 'aiutoma'), '', 'filter_action', false); ?>
            </div>
            <?php
        }
    }
...
trait AbilitiesUi {
    ...
    private function get_ability_array($slug) {
        ...
        if ($provider === 'Plugin') {
            ...
            if (stripos($plugin_name, 'aiutoma') !== false || strtolower($plugin_name) === 'wizard ai' || $ns === 'aiutoma') {
                $plugin_name = 'Aiutoma';
            }
        }
    ...
    private function render_detail_view() {
        ...
        if (empty($ability_slug)) {
            echo '<div class="notice notice-error"><p>' . esc_html__('No ability specified.', 'aiutoma') . '</p></div>';
            return;
        }

        $ability = $this->get_ability_array($ability_slug);
        if (!$ability) {
            echo '<div class="notice notice-error"><p>' . esc_html__('Ability not found.', 'aiutoma') . '</p></div>';
            return;
        }

        $back_url = admin_url('admin.php?page=aiutoma-abilities');
        $test_url = add_query_arg([
            'page'    => 'aiutoma-abilities',
            'action'  => 'test',
        ...
        if ($provider_label === 'Core') $provider_label = __('Core', 'aiutoma');
        if ($provider_label === 'Theme') $provider_label = __('Theme', 'aiutoma');
        if ($provider_label === 'Plugin') {
            $provider_label = !empty($ability['plugin_name']) ? $ability['plugin_name'] : __('Plugin', 'aiutoma');
        }

        ?>
        <div class="ability-explorer-detail">
            <div class="ability-detail-header">
                <a href="<?php echo esc_url($back_url); ?>" class="button"><?php echo wp_kses_post(__('&larr; Back to List', 'aiutoma')); ?></a>
                <a href="<?php echo esc_url($test_url); ?>" class="button button-primary"><?php esc_html_e('Test Ability', 'aiutoma'); ?></a>
            </div>
            ...
            <?php if (!empty($ability['description'])) : ?>
                <div class="ability-detail-section">
                    <h3><?php esc_html_e('Description', 'aiutoma'); ?></h3>
                    <p><?php echo esc_html($ability['description']); ?></p>
                </div>
            <?php endif; ?>

            <div class="ability-detail-section">
                <h3><?php esc_html_e('Details', 'aiutoma'); ?></h3>
                <table class="ability-detail-table">
                    <tr>
                        <th><?php esc_html_e('Provider', 'aiutoma'); ?></th>
            ...
            <?php if (!empty($ability['input_schema'])) : ?>
                <div class="ability-detail-section">
                    <h3><?php esc_html_e('Input Schema', 'aiutoma'); ?></h3>
                    <div class="ability-schema-wrapper">
                        <button type="button" class="button button-small ability-copy-btn" data-copy="input-schema"><?php esc_html_e('Copy', 'aiutoma'); ?></button>
            ...
            <?php if (!empty($ability['output_schema'])) : ?>
                <div class="ability-detail-section">
                    <h3><?php esc_html_e('Output Schema', 'aiutoma'); ?></h3>
                    <div class="ability-schema-wrapper">
                        <button type="button" class="button button-small ability-copy-btn" data-copy="output-schema"><?php esc_html_e('Copy', 'aiutoma'); ?></button>
            ...
            <div class="ability-detail-section">
                <h3><?php esc_html_e('Raw Data', 'aiutoma'); ?></h3>
                <div class="ability-schema-wrapper">
                    <button type="button" class="button button-small ability-copy-btn" data-copy="raw-data"><?php esc_html_e('Copy', 'aiutoma'); ?></button>
    ...
    private function render_test_runner() {
        ...
        <div class="ability-explorer-test-runner">
            <div class="ability-detail-header">
                <a href="<?php echo esc_url($back_url); ?>" class="button"><?php echo wp_kses_post(__('&larr; Back to List', 'aiutoma')); ?></a>
                <a href="<?php echo esc_url($detail_url); ?>" class="button"><?php esc_html_e('View Details', 'aiutoma'); ?></a>
            </div>

            <h2><?php esc_html_e('Test Ability:', 'aiutoma'); ?> <?php echo esc_html($ability['name']); ?></h2>
            <p class="ability-detail-slug"><code><?php echo esc_html($ability['slug']); ?></code></p>
            ...
            <div class="ability-detail-section ability-test-editor">
                <h3><?php esc_html_e('Input Data', 'aiutoma'); ?></h3>
                <?php if (empty($ability['input_schema'])) : ?>
                    <div class="notice notice-warning inline" style="margin: 10px 0;">
                        <p>
                            <strong><?php esc_html_e('No Input Required', 'aiutoma'); ?></strong><br>
                            <?php esc_html_e('This ability does not accept any input parameters. Simply click "Invoke Ability" to execute it.', 'aiutoma'); ?>
                        </p>
                    </div>
                <?php else : ?>
                    <p class="description">
                        <?php esc_html_e('Edit the JSON input below to test the ability. The input will be validated against the input schema if available.', 'aiutoma'); ?>
                    </p>
                    <div class="notice notice-info inline" style="margin: 10px 0;">
                        <p>
                            <strong><?php esc_html_e('How to test:', 'aiutoma'); ?></strong><br>
                            <ol>
                                <li><?php esc_html_e('Edit the JSON input below with your test data', 'aiutoma'); ?></li>
                                <li><?php esc_html_e('Click "Validate Input" to check your JSON is correct', 'aiutoma'); ?></li>
                                <li><?php esc_html_e('Click "Invoke Ability" to execute the ability with your input', 'aiutoma'); ?></li>
                                <li><?php esc_html_e('View the results below', 'aiutoma'); ?></li>
                            </ol>
                        </p>
                    </div>
                <?php endif; ?>

                <label for="ability-test-payload" class="screen-reader-text"><?php esc_html_e('Ability test input (JSON)', 'aiutoma'); ?></label>
                ...
                <div class="ability-test-actions" style="margin-top:10px;">
                    <button type="button" id="ability-test-invoke" class="button button-primary" data-ability="<?php echo esc_attr($ability_slug); ?>">
                        <?php esc_html_e('Invoke Ability', 'aiutoma'); ?>
                    </button>
                    <button type="button" id="ability-test-validate" class="button">
                        <?php esc_html_e('Validate Input', 'aiutoma'); ?>
                    </button>
                    <button type="button" id="ability-test-clear" class="button">
                        <?php esc_html_e('Clear Result', 'aiutoma'); ?>
                    </button>
                </div>
            ...
            <div class="ability-detail-section ability-test-result-container" id="ability-test-result-container" style="display: none;">
                <h3><?php esc_html_e('Result', 'aiutoma'); ?></h3>
                <div id="ability-test-result" class="ability-schema-wrapper"></div>
            </div>

            <?php if (!empty($ability['input_schema'])) : ?>
                <div class="ability-detail-section ability-test-schema">
                    <h3><?php esc_html_e('Input Schema Reference', 'aiutoma'); ?></h3>
                    <div class="ability-schema-wrapper">
                        <button type="button" class="button button-small ability-copy-btn" data-copy="test-input-schema"><?php esc_html_e('Copy', 'aiutoma'); ?></button>
    ...
    public function aiutoma_abilities_page_html() {
        ...
        echo '<h1><span class="dashicons dashicons-superhero"></span> ' . esc_html__('AI Abilities', 'aiutoma') . '</h1>';
        ...
        $abilities_raw = apply_filters('aiutoma/abilities', $abilities_raw);
        $abilities = [];
        $stats = [
            'total' => 0,
        ...
        ?>
            ...
            <p><?php esc_html_e('The following abilities are registered by Aiutoma and other plugins. These abilities can be used by the Editor Agent, Chatbot, and Playground.', 'aiutoma'); ?></p>
            
            <div class="ability-explorer-stats">
                <a href="<?php echo esc_url(admin_url('admin.php?page=aiutoma-abilities')); ?>" class="ability-stat-card">
                    <div class="ability-stat-number"><?php echo absint($stats['total']); ?></div>
                    <div class="ability-stat-label"><?php esc_html_e('Total Abilities', 'aiutoma'); ?></div>
                </a>
                <a href="<?php echo esc_url(admin_url('admin.php?page=aiutoma-abilities&provider=Core')); ?>" class="ability-stat-card">
                    <div class="ability-stat-number"><?php echo absint($stats['by_provider']['Core'] ?? 0); ?></div>
                    <div class="ability-stat-label"><?php esc_html_e('Core', 'aiutoma'); ?></div>
                </a>
                <a href="<?php echo esc_url(admin_url('admin.php?page=aiutoma-abilities&provider=Plugin')); ?>" class="ability-stat-card">
                    <div class="ability-stat-number"><?php echo absint($stats['by_provider']['Plugin'] ?? 0); ?></div>
                    <div class="ability-stat-label"><?php esc_html_e('Plugins', 'aiutoma'); ?></div>
                </a>
                <a href="<?php echo esc_url(admin_url('admin.php?page=aiutoma-abilities&provider=Theme')); ?>" class="ability-stat-card">
                    <div class="ability-stat-number"><?php echo absint($stats['by_provider']['Theme'] ?? 0); ?></div>
                    <div class="ability-stat-label"><?php esc_html_e('Theme', 'aiutoma'); ?></div>
                </a>
            </div>

            <?php
            $table = new Aiutoma_Ability_Table($abilities);
            $table->prepare_items();
            ?>
            <form method="get">
                <input type="hidden" name="page" value="aiutoma-abilities" />
                <?php
                $table->search_box(__('Search Abilities', 'aiutoma'), 'ability');
                $table->display();
                ?>
            </form>
        </div>
