...
trait CacheBypass {
    ...
    public function exclude_api_uri_from_cache($uris) {
        ...
        $uris[] = '/wp-json/aiutoma/.*';
        $uris[] = '/wp-admin/admin-ajax\.php\?action=aiutoma_.*';
        return $uris;
    }
    ...
    public function litespeed_no_cache_api($no_cache) {
        ...
        if (!empty($_SERVER['REQUEST_URI']) && strpos($_SERVER['REQUEST_URI'], '/wp-json/aiutoma/') !== false) {
            return true;
        }
        return $no_cache;
    }
